...
void call_push(node_t *stack, int node_to_push,
               char *name, node_t *nodes_popped[], int *num_nodes_popped) {
  printf("  %s(push): ", (char *)name);
  push(stack, nodes_popped[node_to_push]);
  nodes_popped[node_to_push] = nodes_popped[--*num_nodes_popped];
  nodes_popped[*num_nodes_popped] = NULL;
}

node_t *call_pop(node_t *stack,
                 char *name, node_t *nodes_popped[], int *num_nodes_popped) {
  printf("  %s(pop): ", (char *)name);
  node_t *popped_node = pop(stack);
  if (popped_node != NULL) {
    nodes_popped[(*num_nodes_popped)++] = popped_node;
  }
  return popped_node;
}

void *thread_A_start(void *name) {
  node_t *nodes_popped[3] = {NULL, NULL, NULL};
  int num_nodes_popped = 0;
  int i = 0;
  node_t *last_pop = NULL;
  for (i = 0; i < 3; i++) { // Up to 3 push/pop operatoins
    // If we cannot do a pop or a push,
    while (num_nodes_popped == 0 && stack_len(STACK) == 0) {
        mc_sched_yield(); // ... then yield to another thread until we can.
    }
    if (num_nodes_popped == 0) { // If we cannot do a push:
        call_pop(STACK, name, nodes_popped, &num_nodes_popped);
        continue;
    } else if (stack_len(STACK) == 0) { // If we cannot do a pop
      int node_to_push = mc_choose(num_nodes_popped);
      call_push(STACK, node_to_push,
                name, nodes_popped, &num_nodes_popped);
      continue;
    }
    // Else we can do either a push or a pop
    if (mc_choose(2) == 0) {
      int node_to_push = mc_choose(num_nodes_popped);
      call_push(STACK, node_to_push,
                name, nodes_popped, &num_nodes_popped);
...
// thread_B_start code is now same as thread_A_start:
...
int main() {
  ...
  node_t x = make_node("X"), y = make_node("Y"), z1 = make_node("Z1");
  node_t z2 = make_node("Z2");
  push(STACK, &z2);
  push(STACK, &z1);
  push(STACK, &y);
  push(STACK, &x);
